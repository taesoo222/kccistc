...
interface counter_if (
    ...
    // assertion
    A_RESET_CLEAR :
    assert property (p_reset_clear_counter)
...
class counter_seq_item extends uvm_sequence_item;
    ...
    virtual function string convert2string();
        return $sformatf(
            "reset_n = %b, enable = %b, counter = %d, o_tick = %b",
            rst_n,
            enable,
            counter,
            o_tick
...
class counter_monitor extends uvm_monitor;
    ...
    virtual task run_phase(uvm_phase phase);
        ...
        forever begin
            // count_seq_item new
            c_item =
                counter_seq_item::type_id::create("counter_seq_item", this);
            @(c_if.mon_cb);
...
class counter_scoreboard extends uvm_scoreboard;
    ...
    bit expected_tick = 0;
    int count_pass_cnt, count_fail_cnt = 0;
    int tick_pass_cnt, tick_fail_cnt = 0;
    int skip_cnt = 0;
    ...
    virtual function void write(counter_seq_item c_item);
        ...
        if (!c_item.rst_n) begin
            `uvm_info(get_type_name(), $sformatf("To skip: counter is x "),
                      UVM_NONE)
            skip_cnt++;
            expected_count = 0;
            expected_tick  = 0;
            ...
            if (expected_count == c_item.counter) begin
                `uvm_info(get_type_name(),
                          $sformatf("PASS !! expect:%d,counter:%d",
                                    expected_count, c_item.counter), UVM_HIGH)
                count_pass_cnt++;
            end else begin
                `uvm_error(get_type_name(), $sformatf(
                           "FAIL !! expect:%d,counter:%d",
                           expected_count,
                           c_item.counter
                           ))
                count_fail_cnt++;
            ...
            if (expected_tick == c_item.o_tick) begin
                `uvm_info(get_type_name(),
                          $sformatf("PASS !! expect:%d,tick:%d", expected_tick,
                                    c_item.o_tick), UVM_HIGH)
                tick_pass_cnt++;
            end else begin
                `uvm_error(
                    get_type_name(), $sformatf(
                    "FAIL !! expect:%d,tick:%d", expected_tick, c_item.o_tick))
                tick_fail_cnt++;
            end

            expected_tick = (c_item.enable && (expected_count == 3'd7));
            if (c_item.rst_n && c_item.enable) expected_count++;
        end
    endfunction
    ...
    virtual function void report_phase(uvm_phase phase);
        ...
        `uvm_info("SCB", $sformatf("Result Count Pass:%d", count_pass_cnt),
                  UVM_NONE)
        `uvm_info("SCB", $sformatf("Result Count Fail:%d", count_fail_cnt),
                  UVM_NONE)
        `uvm_info("SCB", $sformatf("Result Skip:%d", skip_cnt), UVM_NONE)
        `uvm_info("SCB", $sformatf("Result Tick Pass:%d", tick_pass_cnt),
                  UVM_NONE)
        `uvm_info("SCB", $sformatf("Result Tick Fail:%d", tick_fail_cnt),
                  UVM_NONE)
    endfunction
